...
export default async function AddLike(active, reply, comment_id, reply_id) {
    const { userId } = auth();
    const { rows } = await sql`SELECT id FROM fb_users WHERE auth_id=${userId}`;
    console.log(comment_id, rows[0].id);
    if (active) {
        if (reply) {
        ...
        if (reply) {
            await sql`DELETE FROM fb_replylikes WHERE reply_id=${comment_id} AND user_id=${rows[0].id} AND id=${reply_id}`;
        } else {
