...
export default async function followers({ params }) {
    const { rows } =
        await sql`SELECT fb_followers.follower_id AS id, fb_users.name, fb_users.bio FROM fb_followers
        JOIN fb_users ON fb_followers.follower_id = fb_users.id
        WHERE followee_id=${params.id}`;
    return (
